...
namespace IsolatedModel_BidirectionChat
{
    public class Functions
    {
        ...
        [Function("WhoIsOnlineInGroup")]
        [SignalROutput(HubName = "Hub")]
        public SignalRMessageAction WhoIsOnlineInGroup([SignalRTrigger("Hub", "messages", "WhoIsOnlineInGroup", "groupName", "message")] SignalRInvocationContext invocationContext, string groupName, string message)
        {
            return new SignalRMessageAction("echoToConnectionId")
            {
                GroupName = groupName,
                Arguments = new object[] { new EchoMessage(invocationContext, message) }
            };
        }
        ...
        {
            return new SignalRMessageAction("newOnlineUser")
            {
        ...
        [SignalROutput(HubName = "Hub")]
        public SignalRMessageAction OnDisconnected([SignalRTrigger("Hub", "connections", "disconnected")] SignalRInvocationContext invocationContext)
        {
            _logger.LogInformation($"{invocationContext.ConnectionId} has disconnected");
            return new SignalRMessageAction("newOfflineUser")
            {
                Arguments = new object[] { new NewConnection(invocationContext.ConnectionId, string.Empty) },
        ...
        public class EchoMessage
        {
            ...
            public EchoMessage(SignalRInvocationContext invocationContext, string message)
            {
                Sender = string.IsNullOrEmpty(invocationContext.UserId) ? string.Empty : invocationContext.UserId;
                ConnectionId = invocationContext.ConnectionId;
                Text = message;
